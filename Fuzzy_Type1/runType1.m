...
%t2fisSim = readt2fis('fist2.t2fis');
Ke=1;
Kd=0.5141;
Ka=0.077;
Kb=7.336;
...
%% We dont need these methods for type 1
% %% TR Method -> KM
...
y_values = y.signals.values;
%u_values = u.signals.values;
...
% figure; 
% plot(time,y_values);
% legend('Y_Values','Y_values');

figure;
plot(time, ref, 'b', time, y_values, 'r');
legend('Ref', 'Y\_Values');
